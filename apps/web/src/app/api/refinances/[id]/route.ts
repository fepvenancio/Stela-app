...
export async function GET(
  ...
  try {
    const db = getD1()
    const offer = await db.getRefinanceOffer(id)
    if (!offer) return errorResponse('Refinance offer not found', 404, request)
    return jsonResponse({ data: offer }, request)
  } catch (err) {
    logError('refinances/[id]', err)
...
export async function DELETE(
  ...
  try {
    ...
    if (normalizeAddress(parsed.data.address) !== normalizeAddress(offerRecord.new_lender as string)) {
      return errorResponse('Not authorized to cancel this offer', 403, request)
    }

    await db.updateRefinanceStatus(id, 'cancelled')
