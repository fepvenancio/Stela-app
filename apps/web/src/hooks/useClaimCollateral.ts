...
export function useClaimCollateral() {
  ...
  const claimCollateral = useCallback(
    async (inscriptionId: bigint) => {
      ...
      try {
        ...
        const call = client.buildClaimCollateral(inscriptionId)

        toast.info('Claiming collateral...')
        ...
        toast.error('Failed to claim collateral', { description: msg })
        throw err
      } finally {
        setIsPending(false)
      }
  ...
  return { claimCollateral, isPending }
}
